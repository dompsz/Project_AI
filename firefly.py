...
class FireflyOptimizer:
    def __init__(self, dist_matrix, num_agents, num_iterations, alpha=0.5, beta0=1, gamma=1):
        self.history = []  # Historia najlepszych tras do animacji
        self.dist_matrix = dist_matrix
        self.num_agents = num_agents
        self.num_iterations = num_iterations
        self.num_points = len(dist_matrix)
        self.alpha = alpha     # Współczynnik losowości (eksploracja)
        self.beta0 = beta0     # Bazowa siła przyciągania
        self.gamma = gamma     # Tłumienie światła, kontroluje zasięg przyciągania

    def _distance(self, a, b):
        # Liczy liczbę różnych pozycji pomiędzy dwiema permutacjami
        return np.sum(np.array(a) != np.array(b))

    def _move_firefly(self, a, b, beta):
        # Porusza świetlika 'a' w stronę 'b'
        a = a.copy()
        for i in range(len(a)):
            if a[i] != b[i] and np.random.rand() < beta:
                j = a.index(b[i])
                a[i], a[j] = a[j], a[i]
        # Dodatkowa losowa zamiana (eksploracja)
    ...
    def optimize(self):
        # Inicjalizacja populacji – losowe trasy
        ...
        for t in range(self.num_iterations):
            ...
            # Zapisujemy najlepszą trasę w tej iteracji
